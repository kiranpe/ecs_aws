...
bootcmd:
  ...
  - exec &> >(tee -a /var/log/user-data.log | logger -t user-data)
    ...
    # Begin Init
    ######################################
    /opt/aws/bin/cfn-init -v --stack {cluster_name} --resource {ecs_config} --region {var.region} --configsets Install

    #################################
    #Subnets
    #################################
    if [ "${region}" = "us-east-1" ] && [ "$environment" = "prod" ];then
     EFS_SUBNET_AZ_A={subnet_1}
     EFS_SUBNET_AZ_B={subnet_2}
     EFS_SUBNET_AZ_C={subnet_3}
    elif [ "${region}" = "us-east-1" ] && [ "$environment" = "dev" ];then
     EFS_SUBNET_AZ_A={subnet_4}
     EFS_SUBNET_AZ_B={subnet_5}
     EFS_SUBNET_AZ_C={subnet_6}
    elif [ "${region}" = "us-east-1" ] && [ "$environment" = "test" ];then
     EFS_SUBNET_AZ_A={subnet_7}
     EFS_SUBNET_AZ_B={subnet_8}
     EFS_SUBNET_AZ_C={subnet_9}
    elif [ "${region}" = "us-west-1" ] && [ "$environment" = "prod" ] || [ "$environment" = "test" ] || [ "$environment" = "dev" ];then
     EFS_SUBNET_AZ_A={subnet_10}
     EFS_SUBNET_AZ_B={subnet_11}
     EFS_SUBNET_AZ_C={subnet_12}
    fi
    ...
    mkdir -p ${var.efs_mountpoint}
    echo $EFS_IP:/ ${var.efs_mountpoint} nfs4 nfsvers=4.1,rsize=1048576,wsize=1048576,hard,timeo=600,retrans=2,_netdev 0 0 >> /etc/fstab
    mount -a -t nfs4
    ...
    #######################################################################
    /opt/aws/bin/cfn-signal -e $? --region ${var.region} --stack ${var.stackname} --resource ${var.asg_group}

    ######################################
    # Encrypt Cloudwatch Logs
    ######################################
    #aws logs associate-kms-key --log-group-name ${LOG_GROUP_NAME} --region ${var.region} --kms-key-id ${CMK_CW}
